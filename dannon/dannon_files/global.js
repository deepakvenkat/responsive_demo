...
if($('meta[content=Menus]').length > 0 || $('meta[content~=menu]').length > 0){
  $(document).ready(function(){

    var menuPath = location.pathname.replace("main.html", "");

    $('#menuFrame')
    .attr('src', 'http://listingsplay.prd.nymetro.com' + menuPath)
    .css({
      'width': '100%',
      'height': '940px',
		...
		for(var i=0; i<ads['schedule'].length; i++){			
			...
			if(currDate >= sD && currDate < eD && validURL){
				...
				$(function(){
					...
					if(ads['schedule'][currAd].tracking2){$("#tracking").append("<img src='" + ads['schedule'][currAd].tracking2 + "' />");}
					if(ads['schedule'][currAd].link){$("#wrap").prepend("<p><a class='skinTakeoverLink ad_link' href='" + ads['schedule'][currAd].link + "' target='_blank' onclick=\"_gaq.push(['_trackEvent', 'Vulture: Skin Takeover', 'Billboard Click']);\">Click Here to Follow Url</a></p>");}
					if(ads['schedule'][currAd].altLink){$("#wrap").prepend("<p><a class='skinTakeoverLink altLink' href='" + ads['schedule'][currAd].altLink + "' target='_blank'>Click Here to Follow Url</a></p>");}
				});
				break;
...
window.nymag.dcads = {
	set: function(){
		...
		this.restaurantFeatures = (document.getElementsByName("nyml_restaurant_special_features").length) ? document.getElementsByName("nyml_restaurant_special_features")[0].content.split(",") : false;
		...
		if(this.restaurantFeatures !== false) this.keywords = this.keywords.concat(this.restaurantFeatures);
...
if (window.nymag.dcads.levels.l1 === 'homepage' && 
    ...
    NYM.dates.inRange(date, 'June 6, 2013 00:00', 'June 7, 2013 00:00')) {
	NYM.config.takeovers.openingCreditsVideo({
		'campaign': 'USA6-6-NYMag',
		'skipMessage': 'Skip to NYMag',
		'frequencyName': 'USA6-6-nymagHP',
		'frequencyValue': 'cw',
		'frequencyInHours': 8,
        'video': 'http://images.nymag.com/images/2/ads/takeovers/graceland6-6.mp4',
	    'link': 'http://ad.doubleclick.net/clk;272518812;98288791;b',
		'track': 'http://ad.doubleclick.net/ad/N4518.NYMag/B7627771.19;sz=1x1;ord=' + nymTimeStamp() + '?'
	});
}

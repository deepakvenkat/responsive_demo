...
NYM.echo = {
    ...
    initStream: function (o) {
        ...
        // displays comments from pre-Grub Street rebranding and allows users to comment on them via old location
        else if (/www.grubstreet.com/.test(document.URL)) {
			try {
				var urlYear = canonicalUrl;
				urlYear = Number(urlYear.slice(urlYear.indexOf(".com/20") + 5, urlYear.indexOf(".com/20") + 9));
				var urlMonth = canonicalUrl;
				urlMonth = Number(urlMonth.slice(urlMonth.indexOf(".com/20") + 10, urlMonth.indexOf(".com/20") + 12));
				if((urlYear == 2013) && (urlMonth == 5)) {
					if(document.getElementsByName("Issue_Date").length > 0) {
						var pubDate = new Date(document.getElementsByName("Issue_Date")[0].getAttribute("content"));
						var turnoverDate = new Date("May 21, 2013");
						if (pubDate < turnoverDate) {
							canonicalUrl = canonicalUrl.replace("www.grubstreet.com", "newyork.grubstreet.com");
						}
					}
				} else if(((urlYear == 2013) && (urlMonth < 5)) || (urlYear < 2013)) {
					canonicalUrl = canonicalUrl.replace("www.grubstreet.com", "newyork.grubstreet.com");
				}
			} catch(err) {}
        } else {
    ...
    loadCommentTouts: function (o) {
      ...
      // fixes comment touts if the article is pre-Grub Street rebrand
      if (/www.grubstreet.com/.test(urlQuery)) {
		try {
			var urlYear = urlQuery;
			urlYear = Number(urlYear.slice(urlYear.indexOf(".com/20") + 5, urlYear.indexOf(".com/20") + 9));
			var urlMonth = urlQuery;
			urlMonth = Number(urlMonth.slice(urlMonth.indexOf(".com/20") + 10, urlMonth.indexOf(".com/20") + 12));
			if((urlYear == 2013) && (urlMonth == 5)) {
				if(document.getElementsByName("Issue_Date").length > 0) {
					var pubDate = new Date(document.getElementsByName("Issue_Date")[0].getAttribute("content"));
					var turnoverDate = new Date("May 21, 2013");
					if (pubDate < turnoverDate) {
						urlQuery = urlQuery.replace("www.grubstreet.com", "newyork.grubstreet.com");
					}
				}
			} else if(((urlYear == 2013) && (urlMonth < 5)) || (urlYear < 2013)) {
				urlQuery = urlQuery.replace("www.grubstreet.com", "newyork.grubstreet.com");
